//
//  Constants.swift
//  Instagram-Demo
//
//  Created by Cristian Sedano Arenas on 01/09/2020.
//  Copyright © 2020 Cristian Sedano Arenas. All rights reserved.
//

import UIKit

struct Constants {
    
    static let cornerRadius: CGFloat = 8.0
    static let padding: CGFloat = 8
}

struct AppImage {
    static let instagramHeader = "instagramHeader"
    static let logo = "logo"
}

struct Identifier {
    static let settingsCell = "SettingsCell"
    static let notificationCell = "NotificationCell"
    static let IGFeedPostCell = "IGFeedPostCell"
    static let IGFeedPostHeaderCell = "IGFeedPostHeaderCell"
    static let IGFeedPostActionsCell = "IGFeedPostActionsCell"
    static let IGFeedPostGeneralCell = "IGFeedPostGeneralCell"
    static let formCell = "FormCell"
    static let photoCell = "PhotoCell"
    static let profileInfoHeaderReusableView = "ProfileInfoHeaderReusableView"
    static let profileTabReusableView = "ProfileTabReusableView"
    static let userFollowCell = "UserFollowCell"
    static let notificationLikeEventCell = "NotificationLikeEventCell"
    static let notificationFollowEventCell = "NotificationFollowEventCell"
}
